...
package org.careye.player.media;
...
import org.careye.player.media.source.AbstractMediaPlayer;
import org.careye.player.media.source.AccessedByNative;
import org.careye.player.media.source.CalledByNative;
import org.careye.player.media.source.DebugLog;
import org.careye.player.media.source.EyeMediaCodecInfo;
import org.careye.player.media.source.IMediaPlayer;
import org.careye.player.media.source.ITrackInfo;
import org.careye.player.media.source.EyeLibLoader;
import org.careye.player.media.source.EyeMediaMeta;
import org.careye.player.media.source.EyeTimedText;
import org.careye.player.media.source.EyeTrackInfo;
import org.careye.player.media.source.MediaInfo;
...
import org.careye.player.media.misc.IAndroidIO;
 ...
 */
public final class EyeMediaPlayer extends AbstractMediaPlayer {
    private final static String TAG = EyeMediaPlayer.class.getName();
     ...
     */
    private static final EyeLibLoader sLocalLibLoader = new EyeLibLoader() {
        @Override
    ...
    public static void loadLibrariesOnce(EyeLibLoader libLoader) {
        synchronized (EyeMediaPlayer.class) {
            if (!mIsLibLoaded) {
                if (libLoader == null)
                    libLoader = sLocalLibLoader;

                libLoader.loadLibrary("MediaF");
                libLoader.loadLibrary("MediaS");
     ...
     * synchronously instantiating a EyeMediaPlayer from a Uri or resource.
     * <p>
     * When done with the EyeMediaPlayer, you should call {@link #release()}, to
     * free the resources. If not released, too many EyeMediaPlayer instances
     * may result in an exception.
     * </p>
     */
    public EyeMediaPlayer() {
     ...
     */
    public EyeMediaPlayer(EyeLibLoader libLoader) {
        initPlayer(libLoader);
    }

    private void initPlayer(EyeLibLoader libLoader) {
         ...
         */
        native_setup(new WeakReference<EyeMediaPlayer>(this));
    }
    ...
    /*
     * Update the EyeMediaPlayer SurfaceTexture. Call after setting a new
     * display surface.
     */
    ...
    @Override
    public EyeTrackInfo[] getTrackInfo() {
        Bundle bundle = getMediaMeta();
        if (bundle == null)
            return null;

        EyeMediaMeta mediaMeta = EyeMediaMeta.parse(bundle);
        if (mediaMeta == null || mediaMeta.mStreams == null)
            return null;

        ArrayList<EyeTrackInfo> trackInfos = new ArrayList<EyeTrackInfo>();
        for (EyeMediaMeta.EyeStreamMeta streamMeta: mediaMeta.mStreams) {
            EyeTrackInfo trackInfo = new EyeTrackInfo(streamMeta);
            if (streamMeta.mType.equalsIgnoreCase(EyeMediaMeta.EYEM_VAL_TYPE__VIDEO)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_VIDEO);
            } else if (streamMeta.mType.equalsIgnoreCase(EyeMediaMeta.EYEM_VAL_TYPE__AUDIO)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_AUDIO);
            } else if (streamMeta.mType.equalsIgnoreCase(EyeMediaMeta.EYEM_VAL_TYPE__TIMEDTEXT)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT);
            }
            trackInfos.add(trackInfo);
        }

        return trackInfos.toArray(new EyeTrackInfo[trackInfos.size()]);
    }
    ...
    /**
     * Releases resources associated with this EyeMediaPlayer object. It is
     * considered good practice to call this method when you're done using the
     * EyeMediaPlayer. In particular, whenever an Activity of an application is
     * paused (its onPause() method is called), or stopped (its onStop() method
     * is called), this method should be invoked to release the EyeMediaPlayer
     ...
     * if a EyeMediaPlayer object is no longer needed may also lead to
    ...
    public MediaInfo getMediaInfo() {
        ...
        try {
            mediaInfo.mMeta = EyeMediaMeta.parse(_getMediaMeta());
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return mediaInfo;
    ...
    private static class EventHandler extends Handler {
        private final WeakReference<EyeMediaPlayer> mWeakPlayer;

        public EventHandler(EyeMediaPlayer mp, Looper looper) {
            super(looper);
            mWeakPlayer = new WeakReference<EyeMediaPlayer>(mp);
        }

        @Override
        public void handleMessage(Message msg) {
            EyeMediaPlayer player = mWeakPlayer.get();
            if (player == null || player.mNativeMediaPlayer == 0) {
                DebugLog.w(TAG,
                        "EyeMediaPlayer went away with unhandled events");
                return;
            }
            ...
            case MEDIA_TIMED_TEXT:
                if (msg.obj == null) {
                    ...
                    EyeTimedText text = new EyeTimedText(new Rect(0, 0, 1, 1), (String)msg.obj);
     ...
     * just uses the EventHandler system to post the event back to the activity_main app
    ...
    private static void postEventFromNative(Object weakThiz, int what,
        ...
        EyeMediaPlayer mp = (EyeMediaPlayer) ((WeakReference) weakThiz).get();
        if (mp == null) {
            return;
        }
    ...
    private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args) {
        ...
        WeakReference<EyeMediaPlayer> weakPlayer = (WeakReference<EyeMediaPlayer>) weakThiz;
        EyeMediaPlayer player = weakPlayer.get();
        if (player == null)
    ...
    public static class DefaultMediaCodecSelector implements OnMediaCodecSelectListener {
        ...
        public String onMediaCodecSelect(IMediaPlayer mp, String mimeType, int profile, int level) {
            ...
            ArrayList<EyeMediaCodecInfo> candidateCodecList = new ArrayList<EyeMediaCodecInfo>();
            ...
            for (int i = 0; i < numCodecs; i++) {
                ...
                for(String type: types) {
                    ...
                    EyeMediaCodecInfo candidate = EyeMediaCodecInfo.setupCandidate(codecInfo, mimeType);
                    if (candidate == null)
                        continue;
            ...
            EyeMediaCodecInfo bestCodec = candidateCodecList.get(0);

            for (EyeMediaCodecInfo codec : candidateCodecList) {
                if (codec.mRank > bestCodec.mRank) {
                    bestCodec = codec;
                }
            }

            if (bestCodec.mRank < EyeMediaCodecInfo.RANK_LAST_CHANCE) {
